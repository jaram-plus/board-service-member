...
import jaram.jaramplus.mopp_service.exception.InvalidTokenException;
...
public class MemberService {
    ...
    public TokenResponse validateToken(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (!jwtUtil.validateToken(refreshToken)) {
            throw new InvalidTokenException("토큰이 유효하지 않습니다.");   //todo 리프레시 토큰이 만료되어서 다시 로그인 필요 401
        }

        Long memberId = jwtUtil.getMemberId(refreshToken);
        String storedRefreshToken = redisService.getRefreshToken(memberId);
        if (storedRefreshToken == null || !storedRefreshToken.equals(refreshToken)) {
            throw new InvalidTokenException("토큰이 유효하지 않습니다.");   //todo 레디스에 저장된 토큰과 불일치 401
        }
